...
#define liq_domain_ID 2					//primary phase index, obtained from GUI
#define vap_domain_ID 3					//secondary phase index, obtained from GUI
...
real Fd(real a);
real Area_dir(real por, real a, real Dp);
real HTC_dir(cell_t c, Thread *t, real Dp);


//for heat transfer involing interface
real Fi(real sl);
real Area_i(real por, real sl, real Dp);
real HTC_boiling(cell_t c, Thread *t_liq, Thread *t_vap, real Tsat, real Ts, real Dp);
real HTC_PB(cell_t c, Thread *t_liq, Thread *t_vap, real Tsat, real Ts);
real HTC_FB(cell_t c, Thread *t_liq, Thread *t_vap, real Tsat, real Ts, real Dp);
...
{
	//save the solid temperature field (UDS results) to UDM-0 at the begining of every iteration
	Thread *t;
	cell_t c;

	real flow_time;											//physical flow time/current time, sec

	//for small-size particles zone 
	t = Lookup_Thread(domain, bed_small_solid_zone_ID);
	...
	//for large-size particles zone 
	t = Lookup_Thread(domain, bed_large_solid_zone_ID);
...
DEFINE_SOURCE(source_sl, c, t, dS, eqn)
{
	...
	Domain *domain;
	Thread *t_solid;
	int zone_id = THREAD_ID(t);							//obtain the index of current fluid zone to determine the corresponding solid zone
    ...
    real Dp;											//particle diameter in the corresponding solid zone, m
	...
	switch(zone_id)
	{
		case bed_small_fluid_zone_ID:
			t_solid = Lookup_Thread(domain, bed_small_solid_zone_ID);
			Dp = Dps;
			break;

		case bed_large_fluid_zone_ID:
			t_solid = Lookup_Thread(domain, bed_large_solid_zone_ID);
			Dp = Dpl;
			break;

		default:
			Error("Error in the direct heat exchange between solid and liquid.\n");
			break;
	}
    ...
    fprintf(fp, "direct source from liquid=%g\n", source);
...
DEFINE_SOURCE(source_sg, c, t, dS, eqn)
{
	...
	switch(zone_id)
	{
		...
		default:
			Error("Error in the direct heat exchange between solid and vapor.\n");
			break;
	}
    ...
    fprintf(fp, "direct source from vapor=%g\n", source);
...
DEFINE_SOURCE(source_solid, c, t, dS, eqn)
{
	...
	int zone_id = THREAD_ID(t);		//obtain the index of current solid zone to determine the corresponding fluid zone
    ...
    switch(zone_id)
	{
		case bed_small_solid_zone_ID:
			t_mix = Lookup_Thread(domain, bed_small_fluid_zone_ID);
		...
		case bed_large_solid_zone_ID:
			t_mix = Lookup_Thread(domain, bed_large_fluid_zone_ID);
			t_liq = THREAD_SUB_THREAD(t_mix, liq_phase_ID); 
			t_vap = THREAD_SUB_THREAD(t_mix, vap_phase_ID);
			Dp = Dpl;
			//source_4: decay heat power
		...
		default:
			Error("Error in the heat source of solid.\n");
			break;
	}
	...
	{
		Asi = Area_i(por, sat, Dp);
		hsi = HTC_boiling(c, t_liq, t_vap, Tsat, Ts, Dp);
		Qsi = -hsi*Asi*(Ts - Tsat);
...
real HTC_boiling(cell_t c, Thread *t_liq, Thread *t_vap, real Tsat, real Ts, real Dp)
{
	...
	h_FB = HTC_FB(c, t_liq, t_vap, Tsat, Ts, Dp);
...
void source_intf_fluid(cell_t c, Thread *t, real *Qil, real *Qig)
{
	...
	int zone_id = THREAD_ID(t);					//obtain the index of current fluid zone to determine the particle size in the corresponding solid zone
	...
	switch(zone_id)
	{
		...
		default:
			Error("Error in the direct heat exchange between interface and fluid.\n");
			break;
	}
